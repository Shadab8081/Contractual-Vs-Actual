...
if manpower_file is not None and st.button("Run reconciliation", type="primary"):
    with st.spinner("Checking GitHub access and fetching current master..."):
        try:
            github_store.validate_token(GITHUB_TOKEN, GITHUB_REPO)
            master_bytes, sha = github_store.get_file(
                GITHUB_TOKEN, GITHUB_REPO, GITHUB_FILE_PATH, GITHUB_BRANCH
            )
        except github_store.GitHubStoreError as e:
            st.error(str(e))
            st.info("Tip: after changing Streamlit Secrets, save the secrets and reboot/redeploy the app so the new token is loaded.")
            st.stop()
        except Exception as e:
            st.error(f"Unexpected GitHub error: {e}")
            st.stop()
    ...
    if st.button("✅ Confirm — set this as the new master"):
        with st.spinner("Committing to GitHub..."):
            try:
                ...
                st.error(f"Unexpected GitHub save error: {e}")
                st.stop()
