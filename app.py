...
if manpower_file is not None and st.button("Run reconciliation", type="primary"):
    with st.spinner("Fetching current master from GitHub..."):
        try:
            ...
            st.info(
                "Check the GitHub repository, file path, branch, and token permissions. "
                f"Current settings: repo='{GITHUB_REPO}', file='{GITHUB_FILE_PATH}', branch='{GITHUB_BRANCH}'."
            )
            st.stop()
...
if st.session_state.plan is not None:
    ...
    for col, proj in zip(cols, ["LP", "UP", "5&6"]):
        ...
        with col:
            ...
            st.caption(
                ...
                f"{len(p['assignments'])} contractual added · "
                f"{len(p.get('additional_assignments', []))} additional · "
                f"{len(p['unplaced'])} manual · {len(p['unmatched'])} unmatched"
            )
    ...
    st.subheader("Where were employees added?")
    for proj in ["LP", "UP", "5&6"]:
        p = plan[proj]
        if p["assignments"]:
            st.markdown(f"**{proj} — Existing contractual vacant slots**")
            for j, v in p["assignments"]:
                st.write(f"• {j['name']} → Row {v['row']} → {v['position']} → Iqama/PP: {j['iqama']}")
        if p.get("additional_assignments"):
            st.markdown(f"**{proj} — Additional manpower**")
            for item in p.get("additional_added_rows", []):
                j = item["employee"]
                st.write(f"• {j['name']} → Row {item['row']} → Additional → {j.get('sap_position') or j.get('assigned_work')} → Iqama/PP: {j['iqama']}")
        if not p["assignments"] and not p.get("additional_assignments"):
            st.caption(f"{proj}: No employee was successfully added.")
