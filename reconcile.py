"""
Reconciles Actual sheets (LP/UP/5&6) against TOTAL MANPOWER.
New positions auto-appended. Smart TOTAL sheet matching. Promotion logic.
"""
...
def find_or_create_total_row(ws_total, position, proj):
    """Find existing row by position name, or create new one."""
    ...
    for proj, sn in SHEET_NAMES.items():
        ...
        for total_row, (onsite, additional) in totals_by_row.items():
            ...
            if before_o != onsite or before_a != additional:
                ...
                log.append(f"Row {total_row} ({fms_label}): Onsite {before_o}->{onsite}, Additional {before_a}->{additional}")
        change_log[proj] = log
    return change_log
